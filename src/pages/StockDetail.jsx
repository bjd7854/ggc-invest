...
export default function StockDetail() {
  ...
  const loadAll = async () => {
    ...
    const [s, h, hd, n] = await Promise.all([
      ...
      supabase.rpc('get_chart_5min', { p_stock_id: stockId, p_hours: 24 }),
      currentTournamentId
  ...
  const chartData = history.map(h => ({
    time: new Date(h.bucket_at).toLocaleString('ko-KR', { month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit' }),
    price: h.close_price
  }))
    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="card p-4 sm:p-6">
        <div className="flex items-center justify-between mb-4">
          <h2 className="font-display text-xl sm:text-2xl">📈 가격 변동 추이</h2>
          <span className="text-xs text-stone bg-gold-50 px-2 py-1 rounded">5분 단위</span>
        </div>
        {chartData.length < 2 ? (
