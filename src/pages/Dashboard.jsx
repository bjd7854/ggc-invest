  ...
  return (
    <div className="space-y-8 sm:space-y-10">
      ...
      <section>
        ...
        {loading ? (
          <div className="card p-10 text-center text-stone">불러오는 중…</div>
        ) : holdings.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
            {holdings.map(h => {
              ...
              const currentRate = h.stock.previous_price 
                ? ((h.stock.current_price - h.stock.previous_price) / h.stock.previous_price) * 100 
                : 0
              ...
              return (
                <Link 
                  to={`/stock/${h.stock.id}`} 
                  key={h.id}
                  className="card p-4 hover:shadow-gold transition-all"
                >
                  <div className="flex items-start justify-between gap-2 mb-3">
                    <div className="min-w-0 flex-1">
                      <div className="font-display text-base sm:text-lg truncate">{h.stock.name}</div>
                      <div className="text-xs text-stone mt-0.5">
                        {h.stock.symbol} · {h.quantity}주
                      </div>
                    </div>
                    <div className={`text-xs font-semibold whitespace-nowrap ${currentRate > 0 ? 'text-up' : currentRate < 0 ? 'text-down' : 'text-stone'}`}>
                      {currentRate >= 0 ? '▲' : '▼'} {Math.abs(currentRate).toFixed(2)}%
                    </div>
                  </div>

                  <div className="grid grid-cols-2 gap-2 text-xs mb-3">
                    <div className="bg-gold-50/40 rounded px-2 py-1.5">
                      <div className="text-stone text-[10px]">평단가</div>
                      <div className="num-display font-semibold mt-0.5">{formatKRW(h.avg_buy_price)}</div>
                    </div>
                    <div className="bg-gold-50/40 rounded px-2 py-1.5">
                      <div className="text-stone text-[10px]">현재가</div>
                      <div className="num-display font-semibold mt-0.5">{formatKRW(h.stock.current_price)}</div>
                    </div>
                  </div>

                  <div className="pt-3 border-t border-gold-500/15">
                    <div className="flex items-baseline justify-between mb-1">
                      <span className="text-xs text-stone">평가액</span>
                      <span className="num-display text-base font-bold">{formatKRW(value)}</span>
                    </div>
                    <div className="flex items-baseline justify-between">
                      <span className="text-xs text-stone">손익</span>
                      <span className={`num-display text-sm font-semibold ${rate > 0 ? 'text-up' : rate < 0 ? 'text-down' : 'text-stone'}`}>
                        {profit >= 0 ? '+' : ''}{formatKRW(profit)} ({rate >= 0 ? '+' : ''}{rate.toFixed(2)}%)
                      </span>
                    </div>
                  </div>
                </Link>
